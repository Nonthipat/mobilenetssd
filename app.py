...
lineaccesstoken = 'NgpIQGT7T2irW7ZQ94qR9ovwukGMST1L3UV3NaFatI1xb7Igxl77crf+hT2alWtrxZg3254QLb1BlabZuI3RNE7utSXKbumOqDUeHcB8XuB55/Bdhzmjw8IXYdWKdhJmMsdZ5AXXyFczJOLV2yp/PQdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี" :
            replyObj = TextSendMessage(text="กีไม่รู้จักคุณ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "กินข้าวยัง" :
            replyObj = TextSendMessage(text="กียังไม่ได้กินเลย")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "ไปเที่ยวกันมั้ย" :
            replyObj = TextSendMessage(text="กีไม่ไป")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://bots.dialogflow.com/line/bfebebe4-07ec-4f24-be23-756d2e8eff61/webhook"
